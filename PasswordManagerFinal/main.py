...
def create_database(): # Creates database in MySQL, schema = 'PasswordManager', tables = users (login) & passwords
    try:
        ...
        # Users table - Should've changed some datatypes
        cursor.execute('''
        ...
        # Passwords table - Should've changed some datatypes
        cursor.execute('''
...
# sign up and log in tabs shown - can switch between at top
def show_signuplogin(active_tab="signup"):
    ...
    if active_tab == "login":
        tabbed_window.select(tabbed_login) # Needed to set default so homepage buttons on first screen work and can be set to the right ones

    # !!SIGNUP TABBED PAGE LAYOUT!! - Should be able to enter details, then be redirected to login page
    ...
    def submitted_login():
        ...
        if user_id:
            root.withdraw() # Hide window, does NOT close window - NEED to remember that deiconify() can show window again in case needed
